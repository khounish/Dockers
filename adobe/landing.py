...
#Read in all tar.gz files from adobe using AWS CLI
S3_copy_str = "aws s3 cp {0} {1}  --recursive --exclude '*' --include '*.tar.gz' --profile=adobe".format(s3_adobe_landing, home)
print S3_copy_str
sh(S3_copy_str)
...
for file in files:
    ...
    try:
        ...
        sh("aws s3 mv {0}{1} {2}{3}/{1} --profile=adobe".format(s3_adobe_landing, base_file, s3_adobe_archive, year_month))
        base_file
